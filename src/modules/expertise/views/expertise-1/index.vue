<template>
    <div class="expertise-1 container-fluid">
                    ...
                    <table :class="{'table table-striped' : true, 'table-mobile' : $detect === 'mobile'}">
                        <thead>
                            <tr>
                                ...
                                <th>Số điện thoại</th>
                                <th>Công ty</th>
                                <th v-if="$detect === 'web'">Thời gian cập nhật</th>
                                <th>Trạng thái</th>
...
import { getExpertise1, } from '@/repositories/expertise-1';
export default {
    data() {
        return {
            optionsStatus: [
    ...
    async created() {
        ...
        this.$eventBus.$on('refresh_list_td1', data => {
            this.getData()
        })

        if(this.page == 1) {
            this.$eventBus.$on('new_td1', data => {
                this.getData()
            })
        }
    },
    methods: {
        ...
        async getData(id_user) {
            ...
            await getExpertise1(query).then(res => {
