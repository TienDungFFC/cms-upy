<template>
    <div class="disbursement container-fluid">
        <div v-if="$detect === 'web'" class="d-flex align-items-end justify-content-end pb-4">
            ...
            <div class="mr-3">
                ...
                >
                <ldc-option
                    v-for="item in optionDisbursement"
                    :key="item.value"
                    :label="item.label"
                    :value="item.value"
                >
        ...
        <div v-if="$detect === 'mobile'">
            <ldc-model-mobile
                ...
                <div slot="content-model">
                    ...
                    <ldc-select-show-option v-model="status" placeholder="Lọc tài khoản">
                        <ldc-option-show 
                            v-for="item in optionDisbursement"
                            :key="item.value"
                            :label="item.label"
            ...
            <div class="col-12 border-bottom">
                <div class="table-responsive">
                    <table :class="{'table table-striped' : true, 'table-mobie': $detect === 'mobile'}">
                        ...
                        <tbody>
                            <tr v-for="(item, i) in disbursements" :key="i">
                                <td>{{ item.loan_id }}</td>
                                <td>
                                ...
                                <td>{{ item.user ? item.user.data.phone : item.name_user }}</td>
                                <td>
                                    {{ item.contract_signing_date }}
                                </td>
                                <td class="text-danger font-weight-bold">
                                    {{ item.disbursement && item.disbursement.loan ? $formatNumber(item.disbursement.loan) : '' }}
                                </td>
                                <td>
                                ...
                                <td 
                                    :class="{
                                        'text-warning' : item.status == 'WAITING_DISBURSEMENT',
                                        'text-success' : item.status == 'PAYING',
                                        'text-danger' : item.status == 'NOT_DISBURSEMENT'
                                    }">
                                    {{ item.status_text }}
                                </td>
                            ...
                            <no-data :data="disbursements" :colspan="8" />
                        </tbody>
                    </table>
                </div>
            </div>
...
<script>
import { getDisbursement } from "@/repositories/disbursement.js";
import { getUsers, detailUser } from "@/repositories/user.js";
import { getPartner, detailPartner } from "@/repositories/partner.js";
import { optionDisbursement } from "@/constants/optionDisbursement.js";
export default {
    data() {
        return {
            dialog: false,
            ...
            companies: [],
            users: [],
            optionDisbursement: optionDisbursement,
    ...
    methods: {
        ...
        setCompany(){
            detailPartner(this.id_business).then((res) => {
        ...
        getData() {
            ...
            getDisbursement(query).then((res) => {
                if(res.data.success === API_STATUS_SUCCESS) {
                    this.disbursements = res.data.data.data;
