<template>
  <div class="container-fluid expertise-2_detail py-3">
        <div class="row">
                ...
                <form>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-12 col-lg-10">
                            ...
                            class="form-control size-input"
                            readonly>
                            <div v-if="id && !information.name_business" class="text-danger mt-2">
        ...
        <div class="p-2 p-lg-5 mt-3 mt-lg-5 d-flex flex-column flex-lg-row bg-light rounded shadow-sm">
            <div class="mr-5" style="width:278px">
        ...
        <div class="d-flex flex-column flex-lg-row px-lg-5 my-5">
            <button @click="checkCIC()" class="btn btn-primary mt-3 mt-lg-0 size-button">Check CIC</button>
        </div>        

        <div v-if="dataCIC" class="bg-light rounded shadow-sm p-lg-5 mt-5">
            <div class="d-flex">
                <div class="cic-label">
                    <p class="font-weight-bold">Họ tên:</p>
                    <p class="font-weight-bold">Tình trạng:</p>
                    <p class="font-weight-bold">Địa chỉ:</p>
                    <p class="font-weight-bold">Khớp số điện thoại:</p>
                </div>
                <div class="cic-value ml-4">
                    <p>{{ dataCIC.name }}</p>
                    <p :class="setColorScore(dataCIC.score)">{{ dataCIC.score_text }}</p>
                    <p>{{ dataCIC.address }}</p>
                    <p>{{ dataCIC.phone_matched ? 'Có' : 'Không' }}</p>
                </div>
            </div>
        </div>
        <div class="bg-light rounded shadow-sm mt-3 mt-lg-5 p-2 p-lg-5">
            <div class="row">
                        ...
                        <div :class="{'form-group d-flex align-items-center mb-0 mt-2 mt-lg-0' : true, 'w-100 justify-content-between' : $detect === 'mobile'}">
                            <div class="position-relative flex-grow-1 text-center text-lg-left">
                                ...
                                <select 
                                    v-else
                                    v-model="loan_limit"
                                    :class="{'px-2' : true, 'is-invalid' : errors.loan_limit, 'select-ios' : !$isAndroid, 'w-100' : $detect === 'mobile'}"
                                    :disabled="type === 'REFUSE'|| !$myCanExpertise()">
                ...
                <div class="col-lg-12 ml-auto mt-3 mt-lg-5">
                    <button class="btn btn-success size-button" :disabled="loading || type === 'REFUSE'"
                    ...
                    <button 
                        class="btn btn-warning text-white size-button" 
                        :style="$detect === 'web' ? 'width:200px' : 'width: 100%'" 
                        :disabled="type === 'REFUSE'|| !$myCanExpertise()" 
                        @click="backToExpertise1()">
                        Chuyển về TĐ1
                    </button>
                </div>
                <div class="col-lg-12 mt-3 mb-0 form-group">
                    <div class="form-group mb-0">
                        <textarea v-model="note_fail" :disabled="type === 'REFUSE'|| !$myCanExpertise()" :class="{'form-control' : true, 'is-invalid' : errors.note_fail}" rows="3" placeholder="Lý do"></textarea>
...
<script>
import { detailExpertise2, checkCIC, passLoan, failLoan, backToExpertise1 } from '@/repositories/expertise-2'
export default {
    components: {
    ...
    async created() {
        await this.getData();
        this.$eventBus.$on('refresh_expertise2', data => {
            this.goList();
        })
    },
    methods: {
        async getData() {          
            await detailExpertise2(this.id).then(res => {
        ...
        checkCIC() {
            checkCIC(this.id).then(res => {
                if(res.data.success === API_STATUS_SUCCESS) { 
                    ...
                    this.$showMessage(res.data.message, 'warning');
                    this.errors.id_card = res.data.message
                }
        ...
        setColorScore(score) {
            if(score >= 350 && score < 500) {
                ...
                return 'text-warning'
            } else if (score >= 600 && score < 801) {
                return 'text-primary'
            } else if (score >= 801 && score < 851) {
                return 'text-success'
            }
        },
        indexMethod(index) {   
        ...
        backToExpertise1() {
            ...
            backToExpertise1(this.id, query).then(res=> {
                this.$showMessage(res.data.message, 'success');
                this.goList();
            }).catch(err => {
